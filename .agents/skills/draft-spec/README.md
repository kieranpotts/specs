...
Creates a `proposal/<slug>` branch from `main`, copies `proposals/TEMPLATE.md` to `proposals/<slug>/README.md`, fills in the metadata header (authors, dates, `Status: PROPOSED`), identifies the specification files that will need editing, opens a **draft pull request** with one type label (`FEATURE` or `QUALITY`), and opens the associated discussion thread.
