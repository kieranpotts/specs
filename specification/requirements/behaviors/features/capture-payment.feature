Feature: [F7] Capture payment for an order
  ...
  @R7
  Scenario: [F7.1] Capture a payment for a checked-out basket
  ...
  @R7
  Scenario: [F7.2] Retrying capture with the same idempotency key does not charge twice
  ...
  @R7 @R8
  Scenario: [F7.3] A declined card yields no captured payment
  ...
  @R7
  Scenario: [F7.4] Card data is never stored by the system
