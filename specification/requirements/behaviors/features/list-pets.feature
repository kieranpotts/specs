Feature: List pets
  ...
  Scenario: Paginate the pet list
    Given an authenticated caller
     ...
     When the caller requests the list of pets with a page size of 10
  ...
  Scenario: Filter by status
    Given an authenticated caller
     When the caller requests pets filtered by status "available"
  ...
  Scenario: Filter by species
    Given an authenticated caller
     When the caller requests pets filtered by species "dog"
     Then every pet in the response belongs to the species "dog"

  Scenario: Filter by price range
    Given an authenticated caller
     When the caller requests pets with a minimum price of 100 and a maximum price of 500
     Then every pet in the response has a price between 100 and 500 inclusive

  Scenario: Empty result set
    Given an authenticated caller
      And no pets match the applied filters
     When the caller requests the list of pets with those filters
