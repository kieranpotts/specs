...
The reservation operations (reserve, release) change state and are therefore not naturally idempotent. To let Partners retry safely after a network failure, the **reserve** operation MUST accept a caller-supplied idempotency key: a retry carrying the same key against the same pet MUST return the result of the original reservation rather than rejecting the pet as already reserved, and MUST NOT create a second hold. The **release** operation is idempotent by nature — releasing a reservation the caller has already released (or that has lapsed) MUST succeed without error, provided the pet is `available` and the caller was the holder.
