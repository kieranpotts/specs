...
Idempotent systems are common in everyday life: an elevator call button, or a
pedestrian crossing request, move the system into a new state on the first
press; subsequent presses, before the request is satisfied, have no further
effect.

Idempotence is particularly valuable in client-server systems. A client cannot
always be certain that its request reached the server — for example, after a
network failure. If the operation is idempotent, the client can safely resend
the request without risking duplicate side effects on the server's state.
