...
_State the minimum throughput the system MUST sustain under normal and peak load conditions. Examples: The system MUST sustain at least 500 API requests per second under normal load without exceeding the [latency](./latency.md) thresholds. The system MUST sustain at least 1,500 API requests per second under peak load, with a graceful degradation in latency rather than hard failure._
