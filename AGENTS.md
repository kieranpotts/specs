...
- `specification/`: Describes the current production system.
  ...
  - `requirements/`: Expected behaviors and qualities. Prescriptive.
    - `behaviors/`: Capabilities exposed and rules enforced.
      ...
      - `interfaces/`: External behavioral contracts.
...
- `DRAFT`: The proposal is being written. The pull request is open as draft and carries only its type label, eg. `FEATURE`. Not yet ready for review.
...
The [`.agents/skills/`](./.agents/skills/) directory provides on-demand skills for managing the proposal workflow. See the [README](./.agents/skills/README.md) for descriptions of the available skills
